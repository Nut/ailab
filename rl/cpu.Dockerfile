...
COPY conda_env_cpu.yml /tmp/conda_env_cpu.yml

# Create conda environment
RUN conda env create -f /tmp/conda_env_cpu.yml
# Pull the environment name out of the environment.yml
RUN echo "source activate $(head -1 /tmp/conda_env_cpu.yml | cut -d' ' -f2)" >> ~/.bashrc
...
CMD source activate ai-lab-hska-rl && xvfb-run -s "-screen 0 600x400x24" \
