...
COPY conda_env_gpu.yml /tmp/conda_env_gpu.yml

# Create conda environment
RUN conda env create -f /tmp/conda_env_gpu.yml
# Pull the environment name out of the environment.yml
RUN echo "source activate $(head -1 /tmp/conda_env_gpu.yml | cut -d' ' -f2)" >> ~/.bashrc
